<template>
  <!-- Header -->
  <header class="d-flex flex-wrap justify-content-center p-4 border-bottom">
    <router-link
    ...
    >
      ...
      <span class="fs-4 text-primary"> Market Minder</span>
    </router-link>
    <ul class="nav nav-pills" v-if="!!isAuthenticated">
      <li class="nav-item">
        <router-link class="nav-link" to="/">Dashboard</router-link>
      </li>
      <li class="nav-item">
        <router-link class="nav-link" to="/">Manage Users</router-link>
      </li>
      <li class="nav-item">
    ...
    <a v-if="isAuthenticated" class="btn" href="#" @click.prevent="logout">Logout</a>
  </header>
</template>
...
let isAuthenticated = storageService.getLocalStorage('auth')
const authStore = useAuthStore()

const logout = async (values) => {
  try {
    await authStore.signout(values)
    setTimeout(() => {
